...
import type { IncomeAnalysisParties, NormalizedBankTransaction, StatementFileRecord } from "../../types/statementAnalysis";
...
import { createRelatedPartyIdentity } from "./relatedPartyClassifier";
...
describe("análise consolidada sanitizada", () => {
  ...
  it("só exclui cônjuge após cadastro explícito", () => {
    const parties: IncomeAnalysisParties = { accountHolder: null, spouses: [createRelatedPartyIdentity("PESSOA BETA")] };
    const result = classifyTransactions([tx("a", { description: "PIX RECEBIDO REM: PESSOA BETA", counterparty: "PESSOA BETA" })], parties);
    expect(result[0].classification).toBe("EXCLUDED_SPOUSE");
  });
